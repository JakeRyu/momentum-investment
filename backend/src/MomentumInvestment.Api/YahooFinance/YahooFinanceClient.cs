...
///   GET https://query1.finance.yahoo.com/v8/finance/chart/{ticker}?range=3y&interval=1d
///
...
public sealed class YahooFinanceClient
{
    ...
    {
        // 3y, not 2y: the window ends today, and a decision needs 12 months of
        // history before its own `asOf`. The client caps `asOf` at 12 months
        // back, so the deepest lookback reaches 24 months — exactly the edge of
        // a 2y window, which is how historical dates used to surface as a 500.
        // 3y leaves a year of headroom while keeping the cache key
        // `asOf`-independent, which a computed period1/period2 window would not.
        var url = $"{_options.BaseUrl}/v8/finance/chart/{Uri.EscapeDataString(ticker)}?range=3y&interval=1d";
